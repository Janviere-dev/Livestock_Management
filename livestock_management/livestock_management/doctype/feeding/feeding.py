...
class Feeding (Document):
    ...
    def on_submit(self):
        self.apply_feeding_cost(delta_sign=1)

    def on_cancel(self):
        self.apply_feeding_cost(delta_sign=-1)

    def apply_feeding_cost(self, delta_sign=1):
        ...
        if self.entry_type == "Group" and self.group_id:
            # Only include active livestock in the group
    ...
    def _update_livestock_history_and_total(self, livestock_name, cost_delta, is_submit=True):
        ...
        current_total = flt(livestock_doc.total_feeding_cost)
        livestock_doc.total_feeding_cost = max(0, current_total + flt(cost_delta))

         # avoid negative on cancel edge-cases
        if livestock_doc.total_feeding_cost < 0:
            livestock_doc.total_feeding_cost = 0


        # 3. Handle the Child Table Row
        if is_submit:
            # Add a new row to the history log on Submit
            new_row = livestock_doc.append("livestock_feeding_log", {})
            new_row.feeding_id = self.name
            new_row.feeding_date = self.feeding_date
            ...
            livestock_doc.set("livestock_feeding_log", [
                row for row in livestock_doc.livestock_feeding_log 
                if row.feeding_id != self.name
            ])
